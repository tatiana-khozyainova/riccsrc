...
uses
  ...
  StdCtrls, ExtCtrls, ComCtrls, ToolWin, Buttons, Grids, BaseDicts,
  ClientCommon, BaseObjects
  {$IFDEF VER150}
  , Variants
  {$ENDIF}
  ;
...
type
  ...
  private
    ...
    function  GetSelectedElementID: integer;
  ...
  protected
    ...
    procedure RefreshSelectedElement;
  public
    { Public declarations }
    ...
    property SelectedElementID: integer read GetSelectedElementID write SetSelectedElementID;
    ...
    // коллекция, соответствуюшая справочнику
    property IDObjects: TIDObjects read FIDObjects write SetIDObjects;
    // выделенный объект коллекции
    property SelectedObject: TIDObject read GetSelectedObject;
    // вызывается при необходимости инициализировать объект из справочника - для того чтобы инициализировать сложное всякое
    property OnInitObject: TNotifyEvent read FOnInitObject write FOnInitObject;
    // определяет, можно ли редактировать при помощи элемента
    property AllowEditing: boolean read FAllowEditing write SetAllowEditing; 
...
begin
  if Selected then
  begin
    FSelectedElementID := Integer(Item.Data);
...
begin
  ...
  begin
    ...
    ClearTable;
    FDictObject := TMainFacade.GetInstance.AllDicts.DictByName[Value];
    ...
    if (FDictObject.RowCount > 0) and (not varIsEmpty(FDictObject.Dict)) then
    begin
  ...
  begin
    ...
    begin
      ...
      if Assigned(FIDObjects) then
        FIDObjects.Add(FDictObject.Columns[0].Value, FDictObject.Columns[1].Value);
    end
    else MessageBox(Self.Handle, 'Не удалось добавить строку!', 'Сообщение', mb_OK + mb_IconError);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Assigned(FIDObjects) then
      begin
        o := FIDObjects.Add(SelectedElementID, SelectedElementName);
        if Assigned(FOnInitObject) then FOnInitObject(o);
      end;
    end
...
begin
  ...
  begin
    ...
    if Assigned(FIDObjects) then
    begin
      o := FIDObjects.ItemsByID[SelectedElementID];
      if Assigned(o) then iResult := FIDObjects.Remove(o);
    end;

    if (iResult > -1) then
    begin
      if (FDictObject.DeleteRow >= 0) then Reload
      else MessageBox(Self.Handle, 'Не удалось удалить строку',
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if Assigned(FDictObject.Columns[i].ReferDict) then
          begin
            TMainFacade.GetInstance.AllDicts.MakeList(Items, FDictObject.Columns[i].ReferDict.Dict);
...
var i: integer;
begin
  FDictObject.Columns[0].Value := lwCurrentDict.Items[0].Caption;

  for i := 0 to FDictObject.Columns.Count - 2 do
    FDictObject.Columns[i].Value := lwCurrentDict.Selected.SubItems[i];
end;
...
procedure TfrmComplexList.SetIDObjects(const Value: TIDObjects);
begin
  if FIDObjects <> Value then
  begin
    FIDObjects := Value;

    if Assigned(FIDObjects.Poster) then
      DictName := FIDObjects.Poster.DataSourceString;
  end;
end;

function TfrmComplexList.GetSelectedObject: TIDObject;
begin
  Result := nil;
  if Assigned(IDObjects) then Result := IDObjects.ItemsByID[SelectedElementID];
end;

procedure TfrmComplexList.RefreshSelectedElement;
var i, j: integer;
begin
  ...
  begin
    ...
    begin
      ...
      for j := 0 to lwCurrentDict.Items[i].SubItems.Count - 1 do
...
function TfrmComplexList.GetSelectedElementID: integer;
begin
  Result := FSelectedElementID;
end;

procedure TfrmComplexList.SetAllowEditing(const Value: boolean);
begin
  FAllowEditing := Value;
  btnInsert.Enabled := FAllowEditing;
  btnEdit.Enabled := FAllowEditing;
  btnDelete.Enabled := FAllowEditing;
end;
