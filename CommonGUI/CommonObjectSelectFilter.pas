...
type
  TfrmObjectSelect = class(TFrame)
    ...
    procedure btnSearchWellClick(Sender: TObject);
//    procedure pnlButtonsClick(Sender: TObject);
  ...
  public
    { Public declarations }
    procedure Reload(AFilterID: integer; SavePreviousValues: boolean; FilterNumbers: OleVariant);
    procedure Prepare;
...
uses CommonFilterContentForm, Facade, StringUtils, CommonWellSearchForm, WellSearchController;
...
procedure TfrmObjectSelect.Reload(AFilterID: integer; SavePreviousValues: boolean; FilterNumbers: OleVariant);
begin
  FilterID := AFilterID;
  if FilterID>-1 then
  begin
    Filters.LoadFilter(FilterID, SavePreviousValues,  FilterNumbers);
  ...
  TMainFacade.GetInstance.SystemSettings.SectionByName['OSF'].SectionValues.Values['FilterValues'] := Filters.CurrentFilter.FilterTextNumbers;
...
begin
  ...
  begin
    ...
    btnSearchWell.Enabled := FilterID = 1;
  end;
end;
...
begin
  ...
  if SelectImmediately then Reload(-1, true, null);
end;
...
var sFilterType, sFilterNumbers: string;
    vFilterNumbers: OleVariant;
begin
  sFilterType := TMainFacade.GetInstance.SystemSettings.SectionByName['OSF'].SectionValues.Values['FilterId'];
  sFilterNumbers := TMainFacade.GetInstance.SystemSettings.SectionByName['OSF'].SectionValues.Values['FilterValues'];

  try
    FilterID := StrToInt(sFilterType);
    vFilterNumbers := Split(sFilterNumbers, ',');
    Reload(FilterID, false, vFilterNumbers);
  except
    Reload(1, False, null);
  end;
end;

procedure TfrmObjectSelect.btnSearchWellClick(Sender: TObject);
var vValues: Variant;
    i: integer;
begin
  if not Assigned(frmWellSearch) then frmWellSearch := TfrmWellSearch.Create(Self);

  if frmWellSearch.ShowModal = mrOK then
  begin
    vValues := varArrayCreate([0, -1], varInteger);
    with frmWellSearch do
    for i := 0 to AreaList.Count - 1 do
    begin
      VarArrayRedim(vValues, varArrayHighBound(vValues, 1) + 1);
      vValues[varArrayHighBound(vValues, 1)] := Integer(AreaList.Objects[i]);
    end;

    if Assigned(Filters.CurrentFilter) then
      Filters.CurrentFilter.FilterValues := vValues;
    Reload(1, false, null);
  end;
end;
