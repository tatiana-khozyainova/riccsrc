...

{
  ...
  function CreateFileByBytes(Bytes:Pointer; Count:Cardinal; FileName:String; ForceCreateDirectory, ForceRewrite: boolean):Boolean;
...
begin
  Result := AFileName;
  for i := 1 to Length(RestrictedCharacters) do
    Result := StringReplace(Result, RestrictedCharacters[i], '_', [rfReplaceAll]);
end;  
...
begin
  ...
  if ForceCreateDirectory then CreatePath(ExtractFilePath(FileName));
