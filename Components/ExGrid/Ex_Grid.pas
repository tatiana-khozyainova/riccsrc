...
uses
  {$IFDEF EX_D7_UP} Themes, UxTheme, {$ENDIF}
...
const
  MaxColumnWidth = MaxInt div 2; // 10000
...
{ TCustomGridColumn }
  ...
  public
    ...
    property MaxWidth: Integer read FMaxWidth write SetMaxWidth default MaxColumnWidth;
...
{ TCustomGridView }
    ...
    GridStyle -            Стиль сетки. Может быть комбинацей из следующих
                           значений:
                             ...
                             gsDotLines -     Рисовать линии сетки точечками
                                              (красиво, но долго).
                             gsFullRowPaint - Продлевает строки на всю клиент.
                                              часть сетки, даже если там нет
                                              колонок
    Header -               Заголовок.
  ...
  TGridStyle = (gsHorzLine, gsVertLine, gsFullHorzLine, gsFullVertLine,
    gsFullRowPaint, gsListViewLike, gsDotLines);
  ...
  TGridDrawHeadersEvent = procedure(Sender: TObject; DrawFixed: Boolean; var DefaultDrawing: Boolean) of object;
  TGridDrawHeaderSectionsEvent = procedure(Sender: TObject; Sections: TGridHeaderSections; AllowFixed: Boolean; var DefaultDrawing: Boolean) of object;
  ...
  private
    ...
    FOnDrawHeaders: TGridDrawHeadersEvent;
    FOnDrawHeaderSections: TGridDrawHeaderSectionsEvent;
...
{$ENDIF}
    ...
    procedure HorzScroll(Sender: TObject; ScrollCode: Integer; var ScrollPos: Integer); virtual;
    procedure KeyDown(var Key: Word; Shift: TShiftState); override;
    procedure KeyPress(var Key: Char); override;
    ...
    procedure VertScroll(Sender: TObject; ScrollCode: Integer; var ScrollPos: Integer); virtual;
  ...
  public
    ...
    procedure DefaultDrawHeaders(DrawFixed: Boolean); virtual;
    procedure DefaultDrawHeaderSections(Sections: TGridHeaderSections; AllowFixed: Boolean);
    ...
    function GetCellRect(Cell: TGridCell): TRect; virtual;
    ...
    property OnDrawHeaders: TGridDrawHeadersEvent read FOnDrawHeaders write FOnDrawHeaders;
    property OnDrawHeaderSections: TGridDrawHeaderSectionsEvent read FOnDrawHeaderSections write FOnDrawHeaderSections;
...
begin
  ...
  begin
    ...
    { абсолютные границы заголовка }
    R.TopLeft := Grid.GetHeaderRect.TopLeft; 
...
begin
  ...
  if Value > MaxColumnWidth then Value := MaxColumnWidth;
  FMaxWidth := Value;
  SetWidth(FWidth);
end;
...
begin
  ...
  if AllowEdit
     and
     {$IFDEF EX_D2009_UP}
     CharInSet(Char(Msg.CharCode), [^H, #32..#255])
     {$ELSE}
     (Char(Msg.CharCode) in [^H, #32..#255])
     {$ENDIF}
     then
  begin
...
{$IFDEF EX_D7_UP}
procedure TCustomGridView.WMNCPaint(var Message: TMessage);
var
  DC: HDC;
  RC, RW: TRect;
begin
  if FThemeXPEnabled and ThemeServices.ThemesEnabled and
    ...
    try
      ...
      DrawThemeBackground(ThemeServices.Theme[teEdit], DC, 0, 0, RW, nil);
      Message.Result := 0;
    finally
      ReleaseDC(Handle, DC);
    end;
...
begin
  ...
  begin
    ...
    with Canvas do
      ...
      begin
        { заливаем поле для каждой строки по отдельности }
...
begin
  ...
  DefDraw := True;
  try
    if Assigned(FOnDrawHeaders) then FOnDrawHeaders(Self, DrawFixed, DefDraw);
  except
    Application.HandleException(Self);
  end;
  { нужна ли отрисовка по умолчанию }
  if DefDraw then DefaultDrawHeaders(DrawFixed);
end;
...
begin
  ...
  DefDraw := True;
  try
    if Assigned(FOnDrawHeaderSections) then
      FOnDrawHeaderSections(Self, Sections, AllowFixed, DefDraw);
  except
    Application.HandleException(Self);
  end;
  { нужна ли отрисовка по умолчанию }
  if DefDraw then DefaultDrawHeaderSections(Sections, AllowFixed);
end;
...
procedure TCustomGridView.DefaultDrawHeaderSections(
  Sections: TGridHeaderSections; AllowFixed: Boolean);
var
  I: Integer;
  S: TGridHeaderSection;
  R, SR: TRect;
